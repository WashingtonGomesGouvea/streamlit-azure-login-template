...
Este módulo permite conectar a arquivos no SharePoint ou OneDrive
usando autenticação app-only (client credentials flow).

IMPORTANTE: O App Registration no Azure precisa ter a permissão
"Files.Read.All" ou "Files.ReadWrite.All" (Application permission,
não delegada) com consentimento do administrador.
"""
...
class SPConnector:
    ...
    1. SharePoint Site:
       - Parâmetros: hostname, site_path, library_name
       - Exemplo: synviagroup.sharepoint.com/sites/servicosclinicos
       
    2. OneDrive do usuário:
       - Parâmetro: user_upn (ex: "usuario@empresa.com")
       - Caminho relativo a Documents/
    
    Uso básico:
    ```python
    # OneDrive
    sp = SPConnector(tenant_id, client_id, client_secret, user_upn="usuario@empresa.com")
    df = sp.read_csv("Pasta/arquivo.csv")
    
    # SharePoint
    sp = SPConnector(tenant_id, client_id, client_secret,
                     hostname="empresa.sharepoint.com",
                     site_path="sites/meusite",
                     library_name="Documents")
    df = sp.read_excel("Pasta/arquivo.xlsx")
    ```
    """
    ...
    def upload_small(self, path: str, content: bytes, overwrite: bool = True):
        """Faz upload de um arquivo pequeno (< 4MB)"""
        rel = quote(self.normalize_path(path), safe="/")
